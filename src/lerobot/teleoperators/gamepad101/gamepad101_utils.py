...
class InputController:
    ...
    def __init__(self, x_step_size=10.0, y_step_size=10.0, z_step_size=20.0):
        """
...
class GamepadController(InputController):
    ...
    def __init__(self, x_step_size=3.0, y_step_size=3.0, z_step_size=6.0, deadzone=0.2):
    ...
    def start(self):
        ...
        pygame.init()
        pygame.display.set_mode((1, 1))  # Needed to capture gamepad events on some platforms
    ...
    def get_deltas(self):
        ...
        try:
            ...
            stick_l.x = self.joystick.get_axis(0)  # Left stick Left/Right
            stick_l.y = self.joystick.get_axis(1)  # Left stick Up/Down

            stick_r.x = self.joystick.get_axis(3)  # Right stick Left/Right 
            stick_r.y = self.joystick.get_axis(4)  # Right stick Up/Down

            # Read DPad
            dpad = vec(0, 0)
            dpad.x, dpad.y = self.joystick.get_hat(0)

            # Read triggers
            trigger_r = 0
            trigger_l = 0 

            trigger_r = self.joystick.get_axis(5)  # Right trigger
            trigger_l = self.joystick.get_axis(2)  # Left trigger

            shoulder_pan = 0 if abs(stick_l.x) < self.deadzone else stick_l.x * self.x_step_size
            shoulder_lift = 0 if abs(stick_l.y) < self.deadzone else -stick_l.y * self.y_step_size #invert y axis for more intuitive control
            elbow_flex = 0 if abs(stick_r.y) < self.deadzone else stick_r.y * self.z_step_size
            #wrist_flex = 0 if abs(stick_r.y) < self.deadzone else stick_r.y * self.z_step_size
            wrist_flex = -dpad.y * self.z_step_size
            wrist_roll = 0 if abs(stick_r.x) < self.deadzone else -stick_r.x * self.z_step_size
            gripper = 0 if abs(trigger_r) < self.deadzone and abs(trigger_l) < self.deadzone else (trigger_r - trigger_l) * self.z_step_size
